...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    #scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=0.1, total_iters=n_epochs)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.8, patience=2, threshold=0.01)
    for epoch in range(1, n_epochs + 1):
        ...
        #TODO: add one_epoch_test every 10 epochs to monitor accuracy
        #if epoch % 10 == 0:
            #test_acc = one_epoch_test(data_loaders["train"], model, loss)
